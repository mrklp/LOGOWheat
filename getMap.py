...
#############################################################################################################################
#                                                                                                                           #
#  Usage: python getMap.py  -r refSeq -i chr6D_+_329839800_329840000 -w  epi_weights.hdf5  -p 16 -o  ./variantscore/        #
#                                                                                                                           #
#############################################################################################################################
'''

opts,args = getopt(sys.argv[1:],'r:i:o:p:w:h',['help'])
...
refNum = seqtonum(refSeq)
testList = []
for i in range(200):
    for j in range(4):
        temp = np.zeros((4),dtype=int)
        temp[j] = 1
        refCopy = copy.deepcopy(refNum)
        refCopy[:,400+i] = temp
        testList.append(refCopy)
...
acgt = {'A': 0,'G': 1, 'C': 2,'T': 3}
for i in range(400,600):
    if refSeq[i] != 'N':
        refTag = (i-400)*5+acgt[refSeq[i]]
        break
...
scoreList =[distance.euclidean(y, y_pred[refTag]) for y in y_pred]
scoreArray = np.around(np.reshape(scoreList,(200,5)).transpose(), decimals=6)

outFile = open(save_path+"Map_%s.txt"%id,"w")
outFile.write(id+"\n")
outFile.write('A,'+','.join(scoreArray[0].astype('str'))+'\n')
outFile.write('G,'+','.join(scoreArray[1].astype('str'))+'\n')
outFile.write('C,'+','.join(scoreArray[2].astype('str'))+'\n')
outFile.write('T,'+','.join(scoreArray[3].astype('str'))+'\n')
outFile.write('N,'+','.join(scoreArray[4].astype('str'))+'\n')
outFile.write(','+','.join(list(refSeq[400:600]))+'\n')
outFile.close()
